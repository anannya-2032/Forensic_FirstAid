CyFi: Forensic First Aid 
Securing the "Golden Hour" with Automated Live Response & Blockchain IntegrityCyFi is a next-generation digital forensics orchestration platform. It solves the "Golden Hour" 
crisis by automating the capture of volatile (RAM) and stable (Disk) evidence via a zero-dependency Go Agent and anchoring the evidence's cryptographic fingerprints onto a 
Blockchain Ledger to ensure an unbreakable chain of custody.
...
1. The Blockchain (Hardhat)Ensure you have Node.js installed.
   npm install --save-dev hardhat
   npx hardhat node
2. The Dashboard (Python)
   pip install streamlit web3 psutil pandas
   streamlit run dashboard/app.py
3. The USB Agent (Go) -Compile the Go agent to run without a terminal window
